...
function Home() {
  const { count: followerCount, views: pageViews } = useFollowerCount();
  ...
  return (
    <>
      ...
      <div className="ticker-wrap">
        <div className="ticker" style={{ animationDuration: '240s' }}>
          {Array.from({ length: 2 }).map((_, loopIdx) => (
            <Fragment key={loopIdx}>
              {ALL_ELEMENTS.map((el) => (
                <div className="ticker-item" key={`${loopIdx}-${el.symbol}`}>
